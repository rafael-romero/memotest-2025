describe('Memotest de Pokemon', () => {
  ...
  describe('Interaccion del juego', () =>{
    ...
    it('deberia mostrar un mensaje de exito al encontrar una pareja', () => {
      cy.getCard('pikachu').eq(0).click();
      cy.getCard('pikachu').eq(1).click();
      cy.contains('Bien hecho son iguales!!!').should('be.visible');
      cy.getCard('pikachu').should('have.length', 2).and('have.class', 'deshabilitada').and('have.class', 'destapada');
    })

    it('deberia volver a tapar las cartas si no son parejas y mostrar un mensaje de fallo', () =>{
      ...
      cy.getCard("pikachu").first().should("not.have.class", "destapada", {timeout:2000});
    ...
    it('deberia resolver el juego al encontrar todas las parejas', () => {
      cy.getByData('iniciar-btn').click();
      ...
      cy.getByData("iniciar-btn").should('be.visible').and('contain', 'Jugamos de nuevo?');
      cy.getByData("texto-intentos").should('not.be.visible');
    })

  })

  // probar los casos para cuando se clickea en la misma carta, clickear en una carta ya resuelta, o clickear muy rapido
  // probar que el contador funcione
})
